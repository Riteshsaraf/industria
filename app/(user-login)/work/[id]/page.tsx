...
export default function ContentPage() {
  ...
  const categorySlug = searchParams.get('id');
    const [open, setOpen] = useState(false);
     ...
     const [client, setClients] = useState([]);
        ...
        // Fetch project
            async function loadProjects(categorySlug:string, pg=1) {
                try {
                    setLoading(true);
                    const res = await fetch(`/api/project/list?page=${pg}&pageSize=6&categorySlug=${categorySlug}`);
  ...
  return (
    <main className="bg-black min-h-screen text-white">
       ...
       <section className="bg-black text-white pb-10 ">
            ...
            <div className="grid grid-cols-[auto_1fr] gap-8 px-8">
