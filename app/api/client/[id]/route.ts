...
async function fetchFormFromDB(id: string) {
    ...
    if (id !== "new") {
        ...
        const backendRes = await fetch(`${backendUrl}/clients/${id}`, {
            method: "GET",
...
export async function GET(req: Request, context: { params: { id: string } | Promise<{ id: string }> }) {
    try {
        ...
        const url = id ==='search' ? `${backendUrl}/clients?search=${search}page=${page}&limit=${pageSize}` : `${backendUrl}/clients?page=${page}&limit=${pageSize}`;
...
export async function POST(req: Request) {
    try {
        ...
        const backendRes = await fetch(`${backendUrl}/clients`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                title,
